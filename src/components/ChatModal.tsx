    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-[640px] max-h-[80vh] bg-gray-800 rounded-xl shadow-lg flex flex-col border border-gray-600">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
          <h3 className="text-lg font-semibold text-white">Chat with {friend.username}</h3>
            ...
            return (
              <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] rounded-lg px-3 py-2 ${mine ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-100'}`}>
                  ...
                  {m.type === 'script_share' && (
                    <div>
                      <div className="font-semibold mb-1">📘 Shared Script ID: {m.scriptId}</div>
                      {!mine && <button onClick={() => collect(m.scriptId)} className="mt-1 text-xs bg-emerald-600 hover:bg-emerald-700 px-2 py-1 rounded">Collect Script</button>}
                    </div>
                  )}
        ...
        <div className="p-3 border-t border-gray-700 space-y-2">
          <div className="flex items-center space-x-2">
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'){ send('text'); } }} placeholder="Enter message..." className="flex-1 bg-gray-700 text-white rounded px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500" />
            <button onClick={()=>send('text')} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">Send</button>
            <div className="relative">
              <button onClick={()=>setShareOpen(o=>!o)} className="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 rounded">Share Script</button>
              {shareOpen && (
                <div className="absolute right-0 mt-2 w-56 max-h-64 overflow-y-auto bg-gray-900 border border-gray-600 rounded shadow-lg p-2 space-y-2 z-10">
                  {(!collectedScripts || collectedScripts.length===0) && <div className="text-gray-400 text-xs">No collections</div>}
          ...
          <p className="text-[10px] text-gray-500">Shared script ID is for demonstration only; frontend can fetch details again.</p>
        </div>
      </div>
    </div>
  );
